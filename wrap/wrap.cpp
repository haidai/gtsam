...
void usage() {
  cerr << "wrap parses an interface file and produces either MATLAB or python wrappers" << endl;
  cerr << "usage: wrap interfacePath moduleName toolboxPath (headerPath|python)" << endl;
  ...
  cerr << "  If instead the last argument is python, creates python wrappers" << endl;
}
...
int main(int argc, const char* argv[]) {
  ...
  if (argc != 5) {
    cerr << "Wrap expects 4 arguments! Arguments given are:\n";
  ...
  try {
    if (strcmp(argv[4], "python") == 0) {
      generate_python_toolbox(argv[1], argv[2], argv[3]);
    } else {
